...
struct Bus {
	...
	uint _busLine;
	uint _busSеаts;
	cstring _busDriver = nullptr;
};

// Functions for creating (constructing) a bus - return nullptr if the id is wrong,
// add the bus to the fleet if it is OK.
//void addBus(const Bus&);
//it is commented out because it is for use in bus.cpp only

Bus* busCtor(int, int seats = 50); // Create a bus with id and given number of seats
...
// Prints information about all the buses:
void busesPrint();
...
void delBus(int);   // Removes a bus from the fleet and destroys it (frees its memory),
                    ...
                    // to a line
//the function converts number from type int to type cstring(char*)
